...
/// Voice call test screen.
/// Fetches a voice token, initializes TwilioVoice, starts or listens for calls.
...
class _VoiceTestScreenState extends State<VoiceTestScreen> {
  ...
  String? _error;
  String? _lastToken;
  // Incoming calls are handled globally by TwilioCallHandler (in main.dart)
  // via the native TwilioIncomingCallActivity — no local subscriptions needed.
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Text(
                      ...
                      'You cannot call yourself! Use the switcher above to set a different identity on each device.',
  ...
  /// One-tap switch between flutter-tester-1 and flutter-tester-2.
  Future<void> _switchIdentity() async {
    ...
    setState(() {
      _voiceReady = false; // Must re-initialize with new identity
      _error = null;
      _lastToken = null;
    });
    if (mounted) {
  ...
  Future<void> _initVoice() async {
    ...
    switch (result) {
      ...
      case TokenSuccess(:final token):
        _lastToken = token;
        try {
          // Pass FCM token so Twilio registers this device for incoming calls
          final fcmToken = NotificationService.instance.currentFcmToken;
          await TwilioVoice.instance.initialize(
            accessToken: token,
            fcmToken: fcmToken,
          );
          setState(() { _loading = false; _voiceReady = true; });
          if (mounted) {
          ...
          setState(() { _loading = false; _error = 'Voice init failed: $e'; });
        }
    }
  }

  Future<void> _makeCall() async {
    // Ensure microphone permission before calling
    final status = await Permission.microphone.request();
    if (!status.isGranted) {
      setState(() {
        _error = 'Microphone permission denied. Please grant it in device settings.';
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _loading = false; _error = 'Call failed: $e'; });
    }
  }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          const Text(
            '• Voice calls use Twilio client identities (not phone numbers)\n'
            '• Device A identity: "flutter-tester-1"\n'
            '• Device A "Call To": "flutter-tester-2"\n'
            '• Device B identity: "flutter-tester-2"\n'
            '• Device B "Call To": "flutter-tester-1"\n\n'
