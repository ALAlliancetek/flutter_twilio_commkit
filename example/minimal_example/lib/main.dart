...
class MinimalExampleApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // TwilioCallHandler handles ALL incoming call scenarios automatically:
      //   • App foreground  → Voice SDK event
      //   • App background  → FCM notification
...
class _HomeScreenState extends State<_HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Twilio CommKit Demo'),
        actions: [
          IconButton(
      ...
      body: ListView(
        ...
        children: [
          ...
          _FeatureTile(
            ...
            title: 'Video Call',
            subtitle: 'Fetch token → Join Twilio Video room\n'
          ...
          _FeatureTile(
            ...
            title: 'Voice Call',
            subtitle: 'Fetch token → Initialize Voice SDK → Make / Receive calls\n'
...
class _ServerStatusCardState extends State<_ServerStatusCard> {
  ...
  Future<void> _pingServer() async {
    ...
    final result = await TwilioTokenService.instance
        .fetchVideoToken(roomName: 'ping-test');
    if (!mounted) return;
    setState(() {
      _testing = false;
      switch (result) {
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...[
            ...
            '2. Run: flutterfire configure (see lib/firebase_options.dart)',
            '3. Start the Node.js token server: cd docs/token_server && npm start',
            '4. Tap ⚙ Settings → set server URL to your machine\'s LAN IP.',
